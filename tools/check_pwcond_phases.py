...
def load_w0_debug_ascii(path: Path) -> np.ndarray:
    ...
    if data.shape[1] != 5:
        raise ValueError(
            f"Expected 5 columns (kz ig m Re Im) but got {data.shape[1]} columns. "
            f"Check that the file format matches the expected output from four.f90."
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python3 check_pwcond_phases.py w0_debug.dat
  python3 check_pwcond_phases.py w0_debug.dat --gauge canonical
  python3 check_pwcond_phases.py w0_debug.dat --gauge pwcond

The debug file should be generated by running PWCOND with:
...
"""
    ...
    parser.add_argument(
        ...
        default="canonical",
        help="Which reference phase pattern to use (default: canonical).",
    )
    ...
    if not path.exists():
        print(f"Error: File '{args.debug_file}' not found.", file=sys.stderr)
        print("\nTo generate the debug file, run PWCOND with:", file=sys.stderr)
        print("  export PWCOND_DEBUG_PHASES=1", file=sys.stderr)
        print("  pwcond.x < input.in > output.out", file=sys.stderr)
        sys.exit(1)
    
    try:
        w0 = load_w0_debug_ascii(path)
    except Exception as e:
        print(f"Error loading debug file: {e}", file=sys.stderr)
        sys.exit(1)
