  ...
  do ign=1, ngpsh
     ...
     do kz=1, nz1
       if (abs(zsl(kz))+eps.le.taunew(4)*alat) then
         ...
         elseif (lb.eq.3) then
            fx1(kz)=fx1(kz)+x1(iz)*0.5d0*zr  ! Boundary for x1
            fx2(kz)=fx2(kz)+x2(iz)*0.5d0*zr  ! Boundary for x2
            call simpson(nmeshs-iz+1,x3(iz),rab(iz),fx3(kz))
            fx3(kz)=fx3(kz)+x3(iz)*0.5d0*zr  ! Boundary for x3
            call simpson(nmeshs-iz+1,x4(iz),rab(iz),fx4(kz))
            fx4(kz)=fx4(kz)+x4(iz)*0.5d0*zr  ! Boundary for x4
            ...
            ! All rho-dependent terms go to zero at the rho=0 boundary
            x1(iz-1) = 0.d0
            x2(iz-1) = 0.d0
            x3(iz-1) = 0.d0
            x4(iz-1) = 0.d0
            x6(iz-1) = 0.d0
            ! Only the pure 1/r^n term is non-zero
            if (iz.eq.1) then
                x5(iz-1) = 0.d0
            else
                x5(iz-1) = beta_boundary / (abs(zsl(kz))**3)
            endif
