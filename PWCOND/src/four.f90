...
!             f_{z^3-3zr^2}, f_{-xz^2}, f_{-yz^2}, f_{-z(x^2-y^2)}, f_{-xyz}, f_{-x(x^2-3y^2)}, f_{-y(3x^2-y^2)}
!
...
implicit none
  ...
  elseif (lb.eq.3) then
     ...
     s2=-tpi/2.d0/sarea*sqrt(21.d0/tpi/8.d0)
     s3=tpi/4.d0/sarea*sqrt(7.d0/fpi)
  endif
  do ig=1, ngper
    do kz=1, nz1
      if (lb.eq.0) then
      ...
      elseif (lb.eq.3) then
        w0(kz,ig,7)=cim*s1*w0(kz,ig,7)
        w0(kz,ig,4)=-3.d0*cim*s1*zsl(kz)*w0(kz,ig,4)
        w0(kz,ig,2)=-cim*s2*(5.d0*zsl(kz)**2*w0(kz,ig,2)-wadd(kz,ig))
        w0(kz,ig,1)=s3*(5.d0*zsl(kz)**3*w0(kz,ig,1)-3.d0*zsl(kz)*wadd2(kz,ig))
        w0(kz,ig,3)=-cim*s2*(5.d0*zsl(kz)**2*w0(kz,ig,3)-wadd(kz,ig))
        w0(kz,ig,5)=-3.d0*cim*s1*zsl(kz)*w0(kz,ig,5)
        w0(kz,ig,6)=cim*s1*w0(kz,ig,6)
      endif
    enddo
  enddo
